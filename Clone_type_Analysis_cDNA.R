...
clone_distribution<-colSums(clone_type_cDNA_num_reduced)
...
table(data_cDNA_long_qc[grep("IGHV3-30_IGHJ5_42_1674",data_cDNA_long_qc$CloneId2),c("clin","time")])
table(data_cDNA_long_qc[grep("IGHV3-30_IGHJ5_42_1674",data_cDNA_long_qc$CloneId2),c("specimen_label")])

####Filter the clone matrix to perform the analysis
id<-match(names(clone_distribution_filter),colnames(clone_type_cDNA_num_reduced))
clone_type_cDNA_num_filter<-clone_type_cDNA_num_reduced[,id]
clone_type_cDNA_num_filter2<-clone_type_cDNA_num_filter[,colSums(clone_type_cDNA_num_filter!=0)>3] ##194 clones

matrix_clones<-cbind(clone_type_cDNA_df[,2],clone_type_cDNA_num_filter2)
set.seed(112233)
...
annotation_col = data.frame(
  ...
  time = as.factor(clone_type_cDNA_df_filter$time),
...
prefix<-"clone"
sufix<-seq(1,dim(clone_type_cDNA_filter2)[2])
my_names<-paste(prefix,sufix,sep=".")
colnames(clone_type_cDNA_filter2)<-my_names
paste(my_names,collapse ="+")

matrix_clones<-cbind(clone_type_cDNA_df_filter[,1:4],clone_type_cDNA_filter2)
paste(my_names,collapse ="+")
...
library(corrplot)
corrplot(cor(clone_type_cDNA_filter2))
