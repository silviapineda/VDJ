...
data<-read.csv("/Users/Pinedasans/VDJ/Data/ClonesInferedAll_95.csv")
...
save(clones_mat,diversity_all,data,file="~/VDJ/Data/Data_all_95.Rdata")
...
###Longitudinal data
fm_null <- lmer(diversity_long_cDNA$clones_all_cDNA ~ clin + time + (1 | Sample_id),data=diversity_long_cDNA,REML = F)
fm_full <- lmer(diversity_long_cDNA$clones_all_cDNA ~  clin*time + (1 | Sample_id) ,data=diversity_long_cDNA,REML = F)
anova(fm_full, fm_null) 
...
save(clone_type_gDNA_df,clone_type_gDNA_num_reduced,diversity_long_gDNA_filter,file="~/VDJ/Data/clonesInferedAll_gDNA_95.Rdata")
...
save(clone_type_cDNA_df,clone_type_cDNA_num_reduced,diversity_long_cDNA_filter,file="~/VDJ/Data/clonesInferedAll_cDNA_95.Rdata")
...
#####
load("~/VDJ/Data/clonesInferedAll_cDNA_95.Rdata")
...
write.csv(common,"common_95.csv",row.names = F)

common<-read.csv("common_95.csv")
...
clones_common_perc<-as.matrix(common3)/clones_gDNA
colnames(clones_common_perc)<-subject
rownames(clones_common_perc)<-subject
#Plot the matrix
xval <- formatC(clones_common_perc, format="f", digits=1)
