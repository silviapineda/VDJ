...
data_cDNA_IGHM<-data_merge[which(data_merge$isotype=="IGHM"),]
data_cDNA_long<-data_cDNA_IGHM[which(data_cDNA_IGHM$clin!="AR" & data_cDNA_IGHM$clin!="pre-AR"),]
reads_clones_annot_long<-reads_clones_annot[which(reads_clones_annot$clin!="AR" & reads_clones_annot$clin!="pre-AR"),]
reads_clones_annot_long_cDNA<-reads_clones_annot_long[which(reads_clones_annot_long$clones_IGHM>100),]
...
for (i in specimen){
  ...
  write.table(get(paste0("edges",i)),paste0("/Users/Pinedasans/VDJ/ResultsAllClones/network_data/long_cDNA_IGHM/edges",i,".txt"),sep="\t",row.names = F)
  write.table(get(paste0("vertex",i)),paste0("/Users/Pinedasans/VDJ/ResultsAllClones/network_data/long_cDNA_IGHM/vertex",i,".txt"),sep="\t",row.names = F)
}
...
tiff("network_vertex_cluster_gini_cDNA_IGHM.tiff",h=2000,w=2000,res=300)
